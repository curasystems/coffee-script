...
(function() {
  ...
  exports.compile = compile = function(code, options) {
    var answer, coffeeFile, currentColumn, currentLine, fragment, fragments, header, js, jsFile, merge, newLines, sourceMap, _j, _len1, sourceMapFile;
    if (options == null) {
      options = {};
    }
    ...
    try {
      ...
      if (options.sourceMap) {
        ...
        sourceMapFile = helpers.baseFileName(options.filename, true) + ".coffee.map";
    ...
    if (options.sourceMap) {
      ...
      sourceHint = "sourceHint="+ coffeeFile;
      js = "//@ " + sourceHint + "\n" + js;

      //location = "sourceMappingUrl="+ sourceMapFile;
      //js = "//@ " + location + "\n" + js;

      v3SourceMap = sourcemap.generateV3SourceMap(sourceMap, options.filename, jsFile);

      //console.log("built map " + options.filename);
      //console.dir(v3SourceMap);
      if(!global.sourceMaps) 
        global.sourceMaps = {}

      global.sourceMaps[options.filename] = {map:v3SourceMap, coffeeFile:options.filename, jsFile:jsFile}
      //console.log("saved global soruceMaps for " + coffeeFile);
      //fs.writeFileSync(sourceMapFile, v3SourceMap);
